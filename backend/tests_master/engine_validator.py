...
def validate_engine() -> Dict[str, Any]:
    ...
    This avoids external network usage and relies on a minimal synthetic prompt.
    """
    try:
        engine = load_engine_instance()
    except EngineLoadError as exc:
        return {"ok": False, "stage": "load_engine", "error": str(exc)}
    ...
    try:
        ...
        for attr in ("process", "run", "handle_request", "execute"):
            if hasattr(engine, attr) and callable(getattr(engine, attr)):
                ...
                _ = method(test_prompt)
                break
    except Exception as exc:  # noqa: BLE001
        return {"ok": False, "stage": "dry_run", "error": f"Engine call failed: {exc}"}
