...
try:
    from nexus.ai.nexus_engine import (
        Engine, EngineConfig, AccessContext, Crypter,
        WebRetriever, WebSource
    )
except Exception:
...
class FakeConnector:
    ...
    def __init__(self, name: str, text: str):
        ...
        self._text = text
    def infer(self, prompt: str, *, history=None, model_name=None, deadline=None) -> Tuple[str, Dict[str, Any]]:
...
class FakeWeb(WebRetriever):
    ...
    def __init__(self):
        self.providers = []
        self.scraper = None
    def search_all(self, query: str, *, k_per_provider=5, want_images=False, max_total=12,
                   deadline=None, request_id=None) -> List[WebSource]:
        return [
            WebSource(
...
def test_engine_run_happy_path():
    ...
    for k in ("answer","winner","winner_ref","participants","code","sources","photos","meta"):
...
def test_engine_verification_failure_when_no_sources():
    class EmptyWeb(FakeWeb):
        def search_all(self, *a, **k): return []
    eng = mk_engine()
    eng.web = EmptyWeb()
