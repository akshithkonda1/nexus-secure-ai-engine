...
class ModelConnector:
    ...
    def __init__(
        ...
        self.timeout = min(float(timeout), _MAX_MODEL_TIMEOUT)
        self.max_retries = int(max_retries)
        self.adapter = (adapter or "openai.chat").lower()
        self._session = session or requests.Session()
        self._circuit = _CircuitBreaker()

        # Security: enforce HTTPS and optional domain allow-list
        if not _is_https(self.endpoint):
            raise ValueError(f"{name} endpoint must be HTTPS")
        ...
        if not allow and not allow_all:
            raise MisconfigurationError("NEXUS_ALLOWED_MODEL_DOMAINS must be configured")
        if allow and not _host_allowed(self.endpoint, allow):
    ...
    def _post(self, payload: Dict[str, Any], *, deadline: Optional[float] = None) -> Dict[str, Any]:
        ...
        if not allowed:
            raise CircuitOpenError(f"{self.name} circuit open, retry in {wait_for:.2f}s")
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if declared and int(declared) > _MAX_MODEL_RESPONSE_BYTES:
                    raise PayloadTooLargeError(
                        f"Response exceeds {_MAX_MODEL_RESPONSE_BYTES} bytes limit"
                    )
                body = _limit_body(resp, max_bytes=_MAX_MODEL_RESPONSE_BYTES)
...
def _first_str(d: Any, keys: Tuple[str, ...]) -> Optional[str]:
    def infer(
        ...
        *,
        ...
        key = self._resolve_adapter()
        fn = self._ADAPTERS.get(key) or self._ADAPTERS.get("generic.json")
        return fn(self, prompt, history, model_name, deadline)  # (text, meta)
...
class DuckDuckGoHTMLSearch(_BaseHTTPProvider):
    ...
    def search(self, query: str, *, k: int = 5, images: bool = False, deadline: Optional[float] = None) -> List[WebSource]:
        ...
        try:
            ...
            return []
        body = _limit_body(r, max_bytes=_MAX_SCRAPE_BYTES)
...
class HtmlScraper:
    ...
    def enrich(self, src: WebSource, *, deadline: Optional[float] = None) -> WebSource:
        ...
        try:
            ...
            if declared and int(declared) > _MAX_SCRAPE_BYTES:
                raise PayloadTooLargeError("Scrape body too large")
            body = _limit_body(r, max_bytes=_MAX_SCRAPE_BYTES)
...
def _boost_for(url: str) -> float:
    ...
    for i in range(max(0, len(parts) - 2)):
        cand = ".".join(parts[i+1:])
        if cand in _DOMAIN_BOOSTS:
            return _DOMAIN_BOOSTS[cand]
    return 1.0
...
class Engine:
    ...
    def __init__(self, connectors: Dict[str, ModelConnector], *,
        ...
        if os.getenv("NEXUS_HEALTH_AUTORUN", "1").lower() not in {"0", "false", "no"}:
    ...
    def _infer_one(
        ...
        *,
        ...
        t0 = time.time()
        try:
            text, _meta = conn.infer(prompt, history=history, model_name=name, deadline=deadline)
            return name, text, round(time.time()-t0, 3)
        except NexusError as exc:
            log.warning(
                "connector_failed",
    ...
    def run(
        ...
        *,
        ...
        if not _GLOBAL_CONCURRENCY_SEMAPHORE.acquire(timeout=_CONCURRENCY_WAIT_SECONDS):
            raise RateLimitExceeded("Engine is at capacity; please retry shortly.")

        scope_key = f"{self.access.tenant_id}:{self.access.user_id}"
        allowed, retry_in = _GLOBAL_RATE_LIMITER.try_acquire(scope_key)
        if not allowed:
            _GLOBAL_CONCURRENCY_SEMAPHORE.release()
            raise RateLimitExceeded(f"Rate limit exceeded. Retry in {retry_in:.2f}s")
        ...
        if effective_deadline_ms is not None:
            effective_deadline_ms = min(effective_deadline_ms, _MAX_DEADLINE_SECONDS * 1000)
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as pool:
                ...
                for f in as_completed(futs):
                    ...
                    if text:
                        answers[name] = text
            if not answers:
                raise ConnectorError("No model produced an answer.")
