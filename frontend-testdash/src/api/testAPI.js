...
export async function getRunResults(runId) {
  const res = await fetch(`${API_BASE}/tests/results/${runId}`);
...
export async function getWarRoom(runId) {
  const res = await fetch(`${API_BASE}/tests/warroom/${runId}`);
  if (!res.ok) throw new Error("War Room fetch failed");
  return res.json();
}
...
export async function fetchReportHtml(runId) {
  const res = await fetch(`${API_BASE}/tests/reports/${runId}`);
