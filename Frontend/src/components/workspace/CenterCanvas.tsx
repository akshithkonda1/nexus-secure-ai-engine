...
export default function CenterCanvas({ mode, className }: CenterCanvasProps) {
  ...
  return (
    <section
      ...
      className={`flex h-full min-h-[60vh] flex-col justify-between overflow-hidden rounded-[32px] bg-gradient-to-b from-[var(--bg-surface)]/70 via-[var(--layer-muted)]/35 to-[var(--bg-app)]/5 p-8 text-[var(--text)] shadow-[0_30px_80px_-65px_rgba(0,0,0,0.9)] backdrop-blur-2xl transition-[background,transform] ${className ?? ""}`}
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[var(--bg-elev)]/80 text-[var(--accent)] shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] ring-1 ring-[var(--line-subtle)]/60">
          <Sparkles className="h-5 w-5" />
        </div>
      </div>
      <div className="mt-8 flex flex-1 items-center justify-center rounded-[28px] bg-gradient-to-b from-[var(--bg-elev)]/40 via-[var(--layer-muted)]/25 to-transparent px-8 py-12 text-[var(--muted)] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-xl">
