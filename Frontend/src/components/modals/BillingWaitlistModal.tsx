...
export function BillingWaitlistModal() {
  ...
  const visible = openKey === "billing-waitlist";
  ...
  async function onSubmit(data: Form) {
    try {
      await fetch("/api/waitlist", {
        ...
        body: JSON.stringify({ type: "billing", ...data }),
      });
      alert("Thanks! You’re on the billing waitlist.");
    } catch {
      localStorage.setItem("nexus_waitlist_billing", JSON.stringify(data));
  ...
  return (
    <div className="fixed inset-0 z-[60] grid place-items-center bg-black/40">
      <div className="w-full max-w-xl rounded-2xl border border-[color:rgba(var(--border))] bg-[rgb(var(--surface))] p-6 shadow-2xl">
        ...
        <p className="mt-1 text-[color:rgba(var(--text)/0.75)]">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-3">
          <label className="block">
            ...
            <textarea {...register("whyNexus", { required: true })} rows={3}
          ...
          <label className="block">
            ...
            <textarea {...register("whyPlatform", { required: true })} rows={3}
              className="mt-1 w-full rounded-xl border border-[color:rgba(var(--border))] bg-[rgb(var(--panel))] p-3" />
          </label>

          <label className="block">
            <span className="text-sm">3) What are you using Nexus for?</span>
            <textarea {...register("useCase", { required: true })} rows={3}
              className="mt-1 w-full rounded-xl border border-[color:rgba(var(--border))] bg-[rgb(var(--panel))] p-3"
          ...
          <div className="mt-4 flex items-center justify-end gap-2">
            <button type="button" onClick={close}
              className="rounded-xl border border-[color:rgba(var(--border))] px-4 py-2">Cancel</button>
            <button type="submit" className="rounded-xl bg-[color:var(--brand)] px-4 py-2 text-white">Join waitlist</button>
          </div>
        </form>
      </div>
    </div>
