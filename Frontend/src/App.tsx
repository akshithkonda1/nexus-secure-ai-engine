...
class AppErrorBoundary extends Component<{ children: ReactNode }, { hasError: boolean }> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="flex min-h-screen flex-col items-center justify-center bg-app px-6 text-center text-app-text">
          <h1 className="text-3xl font-semibold text-silver">Something went wrong</h1>
          <p className="mt-4 max-w-md text-sm text-app-text opacity-80">
            We encountered an unexpected error. Refresh the page to continue exploring Nexus.ai.
          </p>
          <button
            type="button"
            className="mt-6 rounded-full bg-trustBlue px-5 py-2 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:shadow-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-trustBlue focus-visible:ring-offset-2 focus-visible:ring-offset-[color:var(--surface)]"
...
function Placeholder({ title, description }: { title: string; description: string }) {
  return (
    <div className="mx-auto flex min-h-[50vh] max-w-3xl flex-col items-center justify-center rounded-3xl border border-app-border bg-surface p-12 text-center text-app-text shadow-card">
      <h2 className="text-2xl font-semibold text-silver">{title}</h2>
      <p className="mt-4 text-sm text-app-text opacity-80">{description}</p>
  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          <div className="flex min-h-screen flex-col items-center justify-center bg-app px-6 text-center text-app-text">
            <div className="max-w-md rounded-3xl border border-app-border bg-surface p-8 shadow-card">
              <h1 className="text-3xl font-semibold text-silver">Page not found</h1>
              <p className="mt-4 text-sm text-app-text opacity-80">
  ...
  return (
    <AppErrorBoundary>
      <BrowserRouter>
        <Suspense
          fallback={
            <div className="flex min-h-screen items-center justify-center bg-app text-app-text" role="status" aria-live="polite">
