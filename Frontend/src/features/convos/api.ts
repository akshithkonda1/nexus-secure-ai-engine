...
export async function askSSE(prompt: string, headers: Record<string,string>, patch: PatchFn, signal: AbortSignal) {
  ...
  while (true) {
    ...
    for (const block of chunks) {
      ...
      const data = m[1]; if (data === "[DONE]") return;
      try {
        const obj = JSON.parse(data);
        if (obj.delta) { full += obj.delta; patch(full, obj); }
      } catch { full += data; patch(full); }
    }
  }
}

export async function askJSON(prompt: string, headers: Record<string,string>, patch: PatchFn) {
