...
export default function ChatWorkspace() {
  ...
  return (
    <div className="h-full grid grid-rows-[1fr_auto]">
      <div className="p-6 overflow-auto whitespace-pre-wrap">{stream || "Ask Ryuzen…"}</div>
      <form className="p-4 border-t flex gap-2" onSubmit={(e)=>{e.preventDefault(); if(canSend) { send(input); setInput(""); }}}>
        <input className="input flex-1" value={input} onChange={(e)=>setInput(e.target.value)} placeholder="Ask Ryuzen…" />
