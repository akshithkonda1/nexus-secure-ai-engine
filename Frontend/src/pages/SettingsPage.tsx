            ...
            <section>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(260px, 1fr))', gap:'1.25rem', alignItems:'flex-end' }}>
                <div>
                  ...
                  <input type="range" min={60} max={95} step={1} value={cfg.dependableThresholdPct} onChange={e=> setCfg(writeConfig({ dependableThresholdPct: parseInt((e.target as HTMLInputElement).value,10) }))} style={{ width:'100%' }}/>
                ...
                <div>
                  ...
                  <input type="number" min={1} max={120} value={cfg.retentionDays} onChange={e=> setCfg(writeConfig({ retentionDays: Math.max(1, Math.min(120, parseInt((e.target as HTMLInputElement).value||'0',10))) }))} className="chatgpt-pill-button" style={{ width:'100%', textAlign:'left' }}/>
