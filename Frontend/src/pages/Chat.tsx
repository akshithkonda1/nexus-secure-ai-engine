...
export default function ChatPage() {
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden bg-gradient-to-b from-[#0f0f1e] via-[#0a0a14] to-[#05050a]">
      {/* Gradient top glow */}
      <div className="pointer-events-none absolute inset-x-0 top-0 z-0 h-64 bg-gradient-to-b from-[#4e46e5]/20 via-transparent to-transparent" />
      ...
      >
        <div className="mx-auto flex w-full max-w-2xl flex-col gap-4">
          ...
          {sending && (
            ...
            >
              <div className="flex items-center gap-2 rounded-2xl bg-white/10 px-4 py-2 text-sm text-white/70 backdrop-blur-xl">
      ...
      >
        <div className="relative">
          ...
          <div className="absolute inset-0 -m-2 rounded-3xl bg-white/5 backdrop-blur-2xl" />
          
          <ChatInput
            disabled={sending}
            suggestions={suggestions}
            placeholder="Ask Nexus anything…"
            className="relative bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl"
            onSend={({ text, files }) => {
          ...
          >
            {suggestions.map((s, i) => (
              <motion.button
                ...
                className="rounded-full bg-white/10 px-4 py-1.5 text-xs font-medium text-white/80 backdrop-blur-xl transition-all hover:bg-white/20"
              >
                {s}
              </motion.button>
            ))}
